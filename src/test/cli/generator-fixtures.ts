...
test.serial("$ memserver g fixtures | without memserver directory raises error", async (t) => {
  ...
  const folderExistence = await fs.pathExists(`${CWD}/memserver`);
...
test.serial("$ memserver generate fixtures [modelName] works", async (t) => {
  ...
  t.true(!(await fs.pathExists(`${fixturesPath}/likes.js`)));
  ...
  t.true(!(await fs.pathExists(`${fixturesPath}/likes.ts`)));
});
