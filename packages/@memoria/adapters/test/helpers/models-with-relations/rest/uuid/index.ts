...
import generateModels from "../../memory/uuid/index.js";
...
export default function generateRESTModels() {
  ...
  let Server = new Memoria({
    routes() {
      ...
      this.put("/photos/:uuid", async (request) => {
        try {
      ...
      this.get("/photos", async ({ queryParams }) => {
        let photos = queryParams.group_uuid
          ? await Photo.findAll({ group_uuid: queryParams.group_uuid })
          : await Photo.findAll();

        return { photos: Photo.serializer(photos) };
      });

      this.get("/photos/:uuid", async (request) => {
        let photo = await Photo.find(request.params.uuid);

        return { photo: Photo.serializer(photo) };
      });

      this.delete("/photos/:uuid", async (request) => {
        try {
      ...
      this.get("/users/:uuid", async (request) => {
        let user = await User.find(request.params.uuid);

        return { user: User.serializer(user) };
      });

      this.delete("/users/:uuid", async (request) => {
        try {
