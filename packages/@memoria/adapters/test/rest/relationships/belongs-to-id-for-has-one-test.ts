...
module(
  ...
  function (hooks) {
    ...
    module("Relationship fetch tests", function () {
      ...
      test("Models empty relationship reference can turn to promise, due to relationship not existing, than can be retried to fetch correctly", async function (assert) {
        assert.expect(14);
        ...
        let insertedGroup = await RESTGroup.insert({ id: 44, name: "Some group" });
        ...
        assert.deepEqual(group, RESTGroup.peek(44));
        ...
        assert.deepEqual(finalGroup, RESTGroup.peek(44));
      });
    });
    ...
    module("Basic relationship assignment then commit tests", function () {
      ...
      test("Fetched model can remove the relationship before update", async function (assert) {
        ...
        assert.deepEqual(group.photo.toJSON(), RESTPhoto.Cache.get(fetchedPhoto.id).toJSON());
    ...
    module(
      ...
      function () {
        test("When related models reflective relationships are completely cleared it doesnt clear the foreign key, just the relationship(previous pointers) of and to the model", async function (assert) {
          ...
          assert.deepEqual(photo.group.toJSON(), RESTGroup.Cache.get(group.id).toJSON());
    ...
    module(
      ...
      function () {
        ...
        test("Reverse relationship can be built, created, updated, deleted with correct changing relationships in one flow", async function (assert) {
          ...
          assert.deepEqual(firstGroup.photo.toJSON(), RESTPhoto.Cache.get(photo.id).toJSON());
          ...
          assert.deepEqual(secondGroup.photo.toJSON(), RESTPhoto.Cache.get(photo.id).toJSON());
          assert.ok(firstGroup.photo instanceof RelationshipPromise);
