  ...
  module("Success cases", function () {
    ...
    test("$Model.update(attributes) does throw an exception when a model gets updated with an unknown $Model.attribute", async function (assert) {
      ...
      try {
        ...
        assert.equal(
          error.message,
          "is_verified is not a valid attribute for a SQLPhoto partial! Provided { is_verified: false }",
        );
      }
      ...
      try {
        ...
        assert.equal(
          error.message,
          "location is not a valid attribute for a SQLPhotoComment partial! Provided { location: Amsterdam }",
        );
      }
