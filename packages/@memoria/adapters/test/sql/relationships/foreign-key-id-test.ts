...
module("@memoria/adapters | SQLAdapter | Relationships | Foreign key mutation tests(id)", function (hooks) {
  setupMemoria(hooks);
  ...
  module("BelongsTo mutations for OneToOne", function () {
    async function setupTargetModels(context, modelOptions = {}) {
      ...
      let updatedTargetPhoto = await SQLPhoto.update({ id: insertedTargetPhoto.id });
