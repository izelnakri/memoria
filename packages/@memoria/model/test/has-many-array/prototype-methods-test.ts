...
module("@memoria/model | HasManyArray Array.prototype methods", function (hooks) {
  ...
  module("fill", function () {
    test("array.fill(null) resets the array", function (assert) {
      ...
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(4).args, [array, fifthPhoto]);
    });
    ...
    test("array.fill(Model) resets the array and sets the array with one element", function (assert) {
      ...
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.lastCall.args, [newResult, secondPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(4).args, [newResult, firstPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(5).args, [newResult, secondPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(6).args, [newResult, thirdPhoto]);
      ...
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 10);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.lastCall.args, [lastArray, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(7).args, [lastArray, firstPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(8).args, [lastArray, secondPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(9).args, [lastArray, thirdPhotoCopy]);
    });
    ...
    test("array.fill(null, index) resets the array from set index", function (assert) {
      ...
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(2).args, [array, fifthPhoto]);
    });
    ...
    test("array.fill(Model, index) resets the array at set index and adds Model at set index if it isnt exist", function (assert) {
      ...
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(0).args, [array, fourthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(1).args, [array, fifthPhoto]);
    });
    ...
    test("array.fill(Model, index) resets the array at set index and replaces the Model from its instance group in another index", function (assert) {
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 11);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 3);
      ...
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.lastCall.args, [anotherArray, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(3).args, [anotherArray, fourthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(4).args, [anotherArray, fifthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(5).args, [anotherArray, thirdPhotoCopy]);
    });
    ...
    test("array.fill(Model, index, end) resets the array at set index and the end index adds Model do set index if it isnt exist", function (assert) {
      ...
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 2);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.lastCall.args, [array, fifthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(0).args, [array, secondPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(1).args, [array, thirdPhoto]);
    });
    ...
    test("array.fill(Model, index, end) resets the array at set index and end index and replaces the Model from its instance group in another index", function (assert) {
      ...
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(0).args, [array, secondPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(1).args, [array, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(2).args, [array, fifthPhotoCopy]);
    });
  });
  ...
  module("splice", function () {
    ...
    test("emptyHasManyArray.splice(x, y, z) works correctly for possible x and y and z integer values", function (assert) {
      ...
      [-2, 0, 2].forEach((value) => {
        [-2, 0, 2].forEach((secondParam) => {
          ...
          assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, addHasManyRelationshipTargetCount);
          assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 0);
          assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.lastCall.args, [array, firstPhoto]);
        });
      });
    ...
    test("emptyHasManyArray.splice(x, y, z, a, b) works correctly for possible x and y and z integer values and adds z, a & b", function (assert) {
      ...
      [-2, 0, 2].forEach((value) => {
        [-2, 0, 2].forEach((secondParam) => {
          ...
          assert.deepEqual(
            RelationshipUtils.addHasManyRelationshipFor
              .getCalls()
              .slice(-3)
              .map((call) => call.args),
            [
    ...
    test("array.splice(x) works correctly for possible x integer values", function (assert) {
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 10);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 7);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(5).args, [array, fourthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(6).args, [array, fifthPhoto]);

      array = new HasManyArray([firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto]);
      assert.deepEqual(array, [firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 15);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 7);
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 15);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 10);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(7).args, [array, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(8).args, [array, fourthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(9).args, [array, fifthPhoto]);
    });
    ...
    test("array.splice(x, y) works correctly for possible x and y integer values", function (assert) {
      ...
      let array = new HasManyArray([firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto, sixthPhoto]);
      assert.deepEqual(array, [firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto, sixthPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 42);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 0);
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 42);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 2);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(0).args, [array, fifthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(1).args, [array, sixthPhoto]);

      array = new HasManyArray([firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto, sixthPhoto]);
      assert.deepEqual(array, [firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto, sixthPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 48);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 2);
      ...
      array = new HasManyArray([firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto, sixthPhoto]);
      assert.deepEqual(array, [firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto, sixthPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 54);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 4);
    ...
    test("array.splice(x, y, existingInstanceGroupAnotherModel) works correctly for possible x and y and z integer values", function (assert) {
      ...
      [-2, 0].forEach((deleteCount) => {
        ...
        addHasManyRelationshipTargetCount += 5;
        assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, addHasManyRelationshipTargetCount);
        assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, removeHasManyRelationshipTargetCount);
      ...
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(3).args, [array, fifthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(4).args, [array, thirdPhoto]);
      ...
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.lastCall.args, [array, thirdPhotoCopy]);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, removeHasManyRelationshipTargetCount);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(7).args, [array, firstPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(8).args, [array, secondPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(9).args, [array, thirdPhoto]);
      ...
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, removeHasManyRelationshipTargetCount);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.lastCall.args, [array, thirdPhotoCopy]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(12).args, [array, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(13).args, [array, fourthPhoto]);
    });
    ...
    test("array.splice(x, y, z, a, b) works correctly for possible x and y and z integer values and adds z, a & b", function (assert) {
      ...
      [-2, 0].forEach((deleteCount) => {
        ...
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(6).args, [array, anotherPhoto]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(7).args, [array, thirdPhotoCopy]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(8).args, [array, secondPhotoCopy]);
        assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(0).args, [array, thirdPhoto]);
        assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(1).args, [array, secondPhoto]);
      });
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, addHasManyRelationshipTargetCount);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, removeHasManyRelationshipTargetCount);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(23).args, [array, sixthPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(24).args, [array, anotherPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(25).args, [array, thirdPhotoCopy]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(26).args, [array, secondPhotoCopy]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(4).args, [array, fourthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(5).args, [array, fifthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(6).args, [array, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(7).args, [array, secondPhoto]);
      ...
      [-2, 0].forEach((deleteCount) => {
        ...
        assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, addHasManyRelationshipTargetCount);
        assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, removeHasManyRelationshipTargetCount);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(32).args, [array, sixthPhoto]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(33).args, [array, anotherPhoto]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(34).args, [array, thirdPhotoCopy]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(35).args, [array, secondPhotoCopy]);
        assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(8).args, [array, thirdPhoto]);
        assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(9).args, [array, secondPhoto]);
      });
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, addHasManyRelationshipTargetCount);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, removeHasManyRelationshipTargetCount);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(50).args, [array, sixthPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(51).args, [array, anotherPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(52).args, [array, thirdPhotoCopy]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(53).args, [array, secondPhotoCopy]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(12).args, [array, firstPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(13).args, [array, secondPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(14).args, [array, thirdPhoto]);
      ...
      [-2, 0].forEach((deleteCount) => {
        ...
        addHasManyRelationshipTargetCount += 4;
        removeHasManyRelationshipTargetCount += 2;
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(59).args, [array, sixthPhoto]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(60).args, [array, anotherPhoto]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(61).args, [array, thirdPhotoCopy]);
        assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(62).args, [array, secondPhotoCopy]);
        assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(15).args, [array, thirdPhoto]);
        assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(16).args, [array, secondPhoto]);
      });
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, addHasManyRelationshipTargetCount);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, removeHasManyRelationshipTargetCount);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(77).args, [array, sixthPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(78).args, [array, anotherPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(79).args, [array, thirdPhotoCopy]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(80).args, [array, secondPhotoCopy]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(19).args, [array, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(20).args, [array, fourthPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(21).args, [array, secondPhoto]);
    });
    ...
    test("array.splice(x, y, z, a, b) works correctly for possible out of bound x and y ->(99, -99) and z integer values and adds z, a & b", function (assert) {
      ...
      assert.deepEqual(array, [firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 14);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 3);
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 18);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 5);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(14).args, [array, sixthPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(15).args, [array, anotherPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(16).args, [array, thirdPhotoCopy]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(17).args, [array, secondPhotoCopy]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(3).args, [array, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(4).args, [array, secondPhoto]);

      array = new HasManyArray([firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto]);

      assert.deepEqual(array, [firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 23);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 5);
      ...
      assert.deepEqual(array, [firstPhoto, secondPhotoCopy, thirdPhotoCopy, sixthPhoto, anotherPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 27);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 9);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(23).args, [array, sixthPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(24).args, [array, anotherPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(25).args, [array, thirdPhotoCopy]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(26).args, [array, secondPhotoCopy]);
      ...
      assert.deepEqual(array, [firstPhoto, secondPhoto, thirdPhoto, fourthPhoto, fifthPhoto]);
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 32);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 9);
      ...
      assert.equal(RelationshipUtils.addHasManyRelationshipFor.callCount, 36);
      assert.equal(RelationshipUtils.removeHasManyRelationshipFor.callCount, 11);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(32).args, [array, sixthPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(33).args, [array, anotherPhoto]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(34).args, [array, thirdPhotoCopy]);
      assert.deepEqual(RelationshipUtils.addHasManyRelationshipFor.getCall(35).args, [array, secondPhotoCopy]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(9).args, [array, thirdPhoto]);
      assert.deepEqual(RelationshipUtils.removeHasManyRelationshipFor.getCall(10).args, [array, secondPhoto]);
    });
  });
